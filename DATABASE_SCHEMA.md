...
## 🎤 INTERVIEW TEMPLATES (supabase_interview_templates.sql)

### 10. `interview_templates`
**Purpose**: Store AI interview structures and conversation flows
**Why Created**: Replace hardcoded interview prompts with database-driven, versionable templates

| Column | Type | Description |
|--------|------|-------------|
| `id` | SERIAL PRIMARY KEY | Auto-incrementing ID |
| `template_name` | VARCHAR(100) UNIQUE | Template identifier (skincare_au_interview) |
| `title` | VARCHAR(200) | Human-readable title |
| `description` | TEXT | Template purpose and scope |
| `interview_type` | VARCHAR(50) | profile_creation, validation, feedback |
| `system_prompt` | TEXT | Complete AI interviewer instructions |
| `interview_structure` | JSONB | Opening questions, flow type, completion criteria |
| `topic_areas` | JSONB | Structured conversation areas with timing |
| `estimated_duration` | INTEGER | Expected interview length (minutes) |
| `version` | INTEGER | Template version |
| `is_active` | BOOLEAN | Whether template is currently used |

**Business Logic**: 
- Database-driven interview flows vs hardcoded prompts
- Version control for interview evolution
- Multiple interview types for different purposes

---

### 11. `interview_topics`
**Purpose**: Individual conversation topics within interview templates
**Why Created**: Structured topic management with suggested questions and timing

| Column | Type | Description |
|--------|------|-------------|
| `id` | SERIAL PRIMARY KEY | Auto-incrementing ID |
| `template_name` | VARCHAR(100) | Links to interview template |
| `topic_name` | VARCHAR(100) | Topic identifier (category_relationship) |
| `topic_title` | VARCHAR(200) | Human-readable topic title |
| `description` | TEXT | Topic purpose and focus |
| `suggested_questions` | JSONB | Array of suggested questions |
| `estimated_duration` | INTEGER | Time allocation for topic (minutes) |
| `topic_order` | INTEGER | Order in interview flow |
| `is_required` | BOOLEAN | Whether topic must be covered |

**Business Logic**: 
- Structured interview flow management
- Time allocation per topic
- Suggested questions for AI guidance
...
### 14. `questionnaire_usage`
**Purpose**: Track which profiles used which questionnaires with feedback
**Why Created**: Analytics on questionnaire effectiveness and user feedback

| Column | Type | Description |
|--------|------|-------------|
| `id` | SERIAL PRIMARY KEY | Auto-incrementing ID |
| `questionnaire_id` | VARCHAR(100) | Which questionnaire was used |
| `profile_id` | VARCHAR(100) | Which profile used it |
| `interview_session_id` | VARCHAR(100) | Links to interview session |
| `usage_date` | TIMESTAMP | When questionnaire was used |
| `completion_status` | VARCHAR(50) | completed, abandoned |
| `feedback_rating` | INTEGER | 1-5 star rating |
| `feedback_comments` | TEXT | User feedback on questionnaire quality |

**Business Logic**: 
- Usage analytics for questionnaire popularity
- User feedback for questionnaire improvement
